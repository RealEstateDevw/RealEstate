...
searchInput.addEventListener('input', async () => {
    ...
    try {
        const response = await fetch(`/api/mop/search?query=${encodeURIComponent(query)}&limit=10`);
...
function displayResults(data) {
    ...
    data.results.forEach(result => {
        ...
        const roleDisplay = result.type === 'user' ? 'Продажник' : 'Лид';
        const roleIcon = result.type === 'user'
